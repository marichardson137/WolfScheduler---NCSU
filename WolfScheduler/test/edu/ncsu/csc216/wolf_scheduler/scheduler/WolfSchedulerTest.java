...
public class WolfSchedulerTest {
	...
	public void testAddCourseToSchedule() {
		...
		// Attempt to add two courses that conflict with each other
		assertTrue(ws.addCourseToSchedule("CSC 226", "001"));
		Exception exception = assertThrows(IllegalArgumentException.class, () -> ws.addCourseToSchedule("CSC 316", "001"));
		assertEquals("The course cannot be added due to a conflict.", exception.getMessage());
		assertEquals(2, ws.getScheduledActivities().length);
		assertEquals(2, ws.getFullScheduledActivities().length);
		
		// Test two arranged courses
		assertTrue(ws.removeActivityFromSchedule(0));
		assertTrue(ws.addCourseToSchedule("CSC 216", "601"));
		assertTrue(ws.addCourseToSchedule("CSC 217", "601"));
	...
	public void testAddEventToSchedule() {
		...
		// Attempt to add an event that conflicts with an existing event
		Exception exception = assertThrows(IllegalArgumentException.class, () -> ws.addEventToSchedule("Second Event", "M", 545, 800, "details..."));
		assertEquals("The event cannot be added due to a conflict.", exception.getMessage());
		assertEquals(1, ws.getScheduledActivities().length);
		assertEquals(1, ws.getFullScheduledActivities().length);
	}
	...
	public void testGetScheduledActivities() {
		...
		assertTrue(ws.addCourseToSchedule("CSC 116", "003"));
		...
		assertEquals("003", schedule[3][1]);
		assertEquals("Intro to Programming - Java", schedule[3][2]);
		assertEquals("TH 11:20AM-1:10PM", schedule[3][3]);
	}
	...
	public void testGetFullScheduledActivities() {
		...
		assertEquals("tbdimitr", schedule[3][4]);
		assertEquals("TH 11:20AM-1:10PM", schedule[3][5]);
		assertEquals("", schedule[1][6]);
	}
